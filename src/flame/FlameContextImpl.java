...
public class FlameContextImpl implements FlameContext {
	...
	void assignWorkers() throws Exception {
		// use partitioner (add kvs worker, then add flame worker, and call assignPartitions())
		...
		if (kvsWorkersCount == 1){
			...
			for (int i = 0; i < kvsWorkersCount - 1; i++){
				partitioner.addKVSWorker(FlameContext.getKVS().getWorkerAddress(i), FlameContext.getKVS().getWorkerID(i), FlameContext.getKVS().getWorkerID(i + 1));
			}
			partitioner.addKVSWorker(FlameContext.getKVS().getWorkerAddress(kvsWorkersCount - 1), FlameContext.getKVS().getWorkerID(kvsWorkersCount - 1), null);
			partitioner.addKVSWorker(FlameContext.getKVS().getWorkerAddress(kvsWorkersCount - 1), null, FlameContext.getKVS().getWorkerID(0));
		}
		...
		if (latestAssignment == null) throw new UserDefinedException("There's no valid assignment!");
	}

	public String invokeOperation(String inputTable, String operation, byte[] lambda, String zeroElement, String otherInputTable) throws Exception {
		...
		String zeroElementUrl = zeroElement == null ? "" : ("&zeroElement=" + java.net.URLEncoder.encode(zeroElement, "UTF-8"));
		String secondTableUrl = otherInputTable == null ? "" : ("&secondTable=" + java.net.URLEncoder.encode(otherInputTable, "UTF-8"));

		for (int i=0; i<latestAssignment.size(); i++) {
			Partition par = latestAssignment.elementAt(i);
			String fromKeyUrl = par.fromKey == null ? "" : ("&startKey=" + java.net.URLEncoder.encode(par.fromKey, "UTF-8"));
			String toKeyExclusiveUrl = par.toKeyExclusive == null ? "" : ("&toKeyExclusive=" + java.net.URLEncoder.encode(par.toKeyExclusive, "UTF-8"));
			
			final String url = "http://"+par.assignedFlameWorker + operation + "?input=" + java.net.URLEncoder.encode(inputTable, "UTF-8") + "&output=" + java.net.URLEncoder.encode(outputTableName, "UTF-8") + "&kvsMasterIp=" + kvsMaster[0] + "&kvsMasterPort=" + kvsMaster[1] + fromKeyUrl + toKeyExclusiveUrl + zeroElementUrl + secondTableUrl;
	...
	public FlameRDD fromTable(String tableName, RowToString lambda) throws Exception {
		String outputTable = invokeOperation(tableName, "/rdd/fromTable", Serializer.objectToByteArray(lambda), null, null);
