  ...
  final static String VALUE_STRING = "value";
        ...
        while (iter.hasNext()){
          ...
          if (row.columns().contains(FlameWorker.VALUE_STRING)){
            ...
            if (it != null){
              int seq = 0;
              for (String s : it){
                kvs.put(qParamsStrings[1], Hasher.hash(row.key() + String.valueOf(seq++)), FlameWorker.VALUE_STRING, s.getBytes());
              }
            }
          }
        }
    ...
    post("/rdd/foldByKey", (req, res) -> {
      ...
      if (iter != null){
        while (iter.hasNext()){
          ...
          kvs.put(qParamsStrings[1], row.key(), FlameWorker.VALUE_STRING, accumulator == null ? zeroElement.getBytes() : accumulator.getBytes());
        }
      }
        ...
        while (iter.hasNext()){
          ...
          if (row.columns().contains(FlameWorker.VALUE_STRING)){
            if (rand.nextDouble() < f){
              kvs.put(qParamsStrings[1], row.key(), FlameWorker.VALUE_STRING, row.get(FlameWorker.VALUE_STRING).getBytes());
            }
          }
        }
      }
        ...
        while (iter.hasNext()){
          ...
          for (String colName : row.columns()){
            ...
            if (it != null){
              int seq = 0;
              for (String s : it){
                kvs.put(qParamsStrings[1], row.key() + colName + String.valueOf(seq++), FlameWorker.VALUE_STRING, s.getBytes());
              }
            }
          }
        }
      ...
      if (!secondTable.equals("")){
        ...
        if (iter != null){
          while (iter.hasNext()){
            ...
            Row rowR;
            if (!kvs.existsRow(secondTable, rowL.key()) || (rowR = kvs.getRow(secondTable, rowL.key())) == null){
              continue;
            }
            for (String colLName : rowL.columns()){
              for (String colRName : rowR.columns()){
                kvs.put(qParamsStrings[1], rowL.key(), Hasher.hash(colLName+"|||"+colRName), (rowL.get(colLName) + "," + rowR.get(colRName)).getBytes());
              }
            }
          }
        }
    ...
    post("/rdd/fold", (req, res) -> {
      ...
      if (iter != null){
        ...
        kvs.put(qParamsStrings[1], FlameWorker.VALUE_STRING, distinguisher, accumulator == null ? zeroElement.getBytes() : accumulator.getBytes());
      }
