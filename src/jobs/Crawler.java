...
public class Crawler {
	...
	public static void run(FlameContext ctx, String[] args) throws Exception {
		...
		// adding a count for table being processed for current round (used to save page content)
		long countRounds = 0;
...
//				}
		...
		// trigger the indexer work here for the current round's table (only those with page)
	...
	static List<String> findUrl(KVSClient kvs, String s, String originalUrl, List<String> rules) throws Exception {
		...
		while (m.find()) {
			...
			&& (m.group(3) == null || m.group(3).length() == 0)) {
				continue;
			}
		...
		// save the outdegrees of current url to outdegrees table (value is comma separated, to be used in pageranks)
		sb.setLength(0);
