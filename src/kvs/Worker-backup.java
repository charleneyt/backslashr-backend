...
public class Worker2 extends generic.Worker {
	final static byte[] LFbyte = new byte[] { (byte) 0x0a };
	final static String LF = "\n";

	// tableName: {rowName : byteOffset}
	Map<String, ConcurrentSkipListMap<String, Long>> tablesWithOffset; // updated to save memory and reduce initial load
																		// time per Ed post #766
	String directory;
	...
	public Worker2(String id, int port, String masterAddr, String directory) {
		super(id, port);
	...
	private void initializeTables() {
					...
					while (input.available() > 0) {
						try {
							...
							if (debugMode) {
								...
								fw2.write("error in initializing tables, table name is: " + tableName
										+ "; starting byte: " + startByte + "\n");
								fw2.close();
							}
						}
					}
		...
		if (debugMode) {
			...
			try {
				fw = new FileWriter("kvsWorker_log", true);
				fw.write("End writing table at: " + System.currentTimeMillis() + "; took "
		...
		while (true) {
			if (numRead == buffer.length) {
				if (debugMode) {
					...
					fw.write("error in readStringSpace, not enough space in buffer, so far read is: "
							+ buffer.toString() + "\n");
					fw.flush();
					fw.close();
				}
...
//      System.out.println("from findRow, tablename is: " + tableName + ", rowName is: " + rowName);
		...
		if (theKey == null) {
			if (debugMode) {
				try {
					...
					fw.write("From findRow, failed to find row, table name is: " + tableName + "; requested row is: "
							+ rowName + "; bytePos is: " + bytePos + "\n");
					fw.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
			...
			if (keyOrMarker == null) {
				if (debugMode) {
					...
					try {
						...
						fw.write("table name is: " + tableName + "; requested row is: " + rowName
								+ "; returned row is: " + newRow.key() + " bytePos is: " + bytePos + "\n");
						fw.close();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
				...
				try {
					...
					if (debugMode) {
						...
						try {
							...
							fw.write("error in findRow," + "Premature end of stream while reading value for key '"
									+ keyOrMarker + "' (read " + bytesRead + " bytes, expecting " + len + ")" + "\n");
							fw.close();
						} catch (Exception e2) {
							// TODO Auto-generated catch block
							e2.printStackTrace();
	...
	public static void main(String[] args) throws IOException {
		...
		port(port);
		Worker2 worker = new Worker2(id, port, masterAddr, args[1]);
		...
		put("/clean/:table", (req, res) -> {
			...
			if (!worker.tablesWithOffset.containsKey(tableName)) {
				...
				if (worker.debugMode) {
					FileWriter fw = null;
					try {
						fw = new FileWriter("404_log", true);
						fw.write("PUT /clean/:table tablename = " + tableName + " time = " + System.currentTimeMillis()
								+ "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
...
//          System.out.println("from put /data/:table/:row/:col, table name is: " + tableName + ", rowName is: " + rowName + ", colName is: " + colName + "\n");
			...
			if (row == null) {
				if (worker.debugMode) {
					FileWriter fw = new FileWriter("missing_row_log", true);
					fw.write("Table: " + tableName + ", row: " + rowName + ", colName: " + colName + " time = "
			...
			if (worker.debugMode) {
				FileWriter fw = null;
				try {
					fw = new FileWriter("put_log", true);
					fw.write("PUT /data/:table/:row/:col tablename = " + tableName + " row = " + rowName + " col = "
							+ colName + " size = " + worker.tablesWithOffset.size() + " time = "
							+ System.currentTimeMillis() + "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
			...
			if (worker.debugMode) {
				...
				fw.write("put /data/:table/:row/:col: table is: " + tableName + ", row is: " + rowName + ", col is: "
						+ colName + "at time " + System.currentTimeMillis() + "\n");
				fw.close();
			}
...
//          System.out.println("from get /data/:table/:row/:col, " + tableName + ", " + rowName +", " + colName + "\n");
			...
			if (targetRow == null) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("404_log", true);
						fw.write("GET /data/:table/:row/:col tablename = " + tableName + " row =  " + rowName
								+ " col = " + colName + " time = " + System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
					}
			...
			if (targetCol == null) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("404_log", true);
						fw.write("GET2 /data/:table/:row/:col tablename = " + tableName + " row =  " + rowName
								+ " col = " + colName + " time = " + System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
			...
			if (worker.debugMode) {
				FileWriter fw = null;
				try {
					fw = new FileWriter("get_log", true);
					fw.write("GET /data/:table/:row/:col tablename = " + tableName + " row = " + rowName + " col= "
							+ colName + " time = " + System.currentTimeMillis() + "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
				}
		...
		get("/", (req, res) -> {
			...
			for (Map.Entry<String, ConcurrentSkipListMap<String, Long>> tableEntry : worker.tablesWithOffset
					.entrySet()) {
...
//                      }
			...
			if (worker.debugMode) {
				...
				try {
					fw = new FileWriter("get_log", true);
					fw.write("GET /view/:table tablename = " + tableName + " time = " + System.currentTimeMillis()
							+ "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
		...
		get("/data/:table/:row", (req, res) -> {
			...
			if (targetRow != null) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("get_log", true);
						fw.write("GET /data/:table/:row/ tablename = " + tableName + " row = " + rowName + " time = "
								+ System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
...
//          System.out.println("/data/:table"+ req.queryParams()+" " + req.url());
			...
			if (!worker.tablesWithOffset.containsKey(tableName)) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("404_log", true);
						fw.write("GET /data/:table tablename = " + tableName + " time = " + System.currentTimeMillis()
								+ "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
			...
			if (hasStartRow && hasEndRow && startRow.compareTo(endRow) > 0) {
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("get_log", true);
						fw.write("GET /data/:table " + tableName + " 400 Bad Request " + "start = " + startRow
								+ " endRow " + endRow + " time = " + System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
					}
			...
			while (input.available() > 0) {
				...
				if (row != null) {
					...
					if (worker.debugMode) {
						...
						try {
							fw = new FileWriter("put_log", true);
							fw.write("PUT /data/:table tablename = " + tableName + " size = "
									+ worker.tablesWithOffset.size() + " time = " + System.currentTimeMillis() + "\n");
							fw.close();
						} catch (IOException e1) {
							e1.printStackTrace();
						}
					...
					currStream.write(Worker2.LFbyte);
				}
			}
			currStream.flush();
		...
		put("/rename/:table", (req, res) -> {
			...
			if (!worker.tablesWithOffset.containsKey(oldTable)) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("404_log", true);
						fw.write("GET /rename/:table old tablename = " + oldTable + " time = "
								+ System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
			...
			if (worker.debugMode) {
				...
				try {
					fw = new FileWriter("put_log", true);
					fw.write("PUT /rename/:table new tablename = " + newTable + " time = " + System.currentTimeMillis()
							+ "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
			...
			if (worker.debugMode) {
				...
				try {
					fw = new FileWriter("put_log", true);
					fw.write("PUT /rename/:table old tablename = " + oldTable + " time = " + System.currentTimeMillis()
							+ "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
		...
		put("/delete/:table", (req, res) -> {
			...
			if (!worker.tablesWithOffset.containsKey(tableToDelete)) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("put_log", true);
						fw.write("PUT /delete/:table tableToDelete = " + tableToDelete + " time = "
								+ System.currentTimeMillis() + "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
			...
			if (worker.debugMode) {
				...
				try {
					fw = new FileWriter("put_log", true);
					fw.write("PUT /delete/:table old tablename = " + tableToDelete + " time = "
							+ System.currentTimeMillis() + "\n");
					fw.close();
				} catch (IOException e1) {
					e1.printStackTrace();
		...
		get("/count/:table", (req, res) -> {
			...
			if (!worker.tablesWithOffset.containsKey(tableName)) {
				...
				if (worker.debugMode) {
					...
					try {
						fw = new FileWriter("put_log", true);
						fw.write("GET /count/:table tablename = " + tableName + " time = " + System.currentTimeMillis()
								+ "\n");
						fw.close();
					} catch (IOException e1) {
						e1.printStackTrace();
