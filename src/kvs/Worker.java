		...
		while (true) {
			...
			if ((b < 0) || (b == 10)) {
				if (debugMode) {
					...
					fw.write("From readStringSpace, b is " + b + "string is "
	...
	public static void main(String[] args) throws IOException {
		...
		put("/clean/:table", (req, res) -> {
			synchronized (worker.tablesWithOffset) {
				...
				BufferedOutputStream newStream = new BufferedOutputStream(
						Files.newOutputStream(tempFile, CREATE, APPEND));
				...
				worker.streams.put(tableName,
						new BufferedOutputStream(Files.newOutputStream(currFile, CREATE, APPEND)));
				return "OK";
			}
		...
		put("/data/:table/:row/:col", (req, res) -> {
			synchronized (worker.tablesWithOffset) {
				...
				if (worker.debugMode) {
					...
					fw.write("put /data/:table/:row/:col: table is: " + tableName + ", row is: " + rowName
							+ ", col is: " + colName + "at time " + System.currentTimeMillis() + "\n");
					fw.close();
				}
