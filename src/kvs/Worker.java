...
public class Worker extends generic.Worker {
	...
	Map<String, TreeMap<String, Row>> tables; //TODO: Ed post 372, row keys case sensitive and we sort with case sensitive
	String directory;
	...
	// EC 1 Every 10 seconds, check whether the worker has received any requests during the past 10 seconds. If it has not, iterate through all the tables, write a new log file for each table – initially under a different name – that contains only entries for the rows that are currently in the table, and then atomically replace the current log file for that table with the newly written one. 
	...
	// EC 2 Replication: download the current list of workers from the master every five seconds, whenever a worker that is currently responsible for a given key receives a PUT for that key, it should forward the PUT to the two workers with the next-higher IDs
        ...
        public void run() {
			try {
                ...
                while (true){
					...
					// 	// no need to sort here, since master maintains and sends the sorted ver via ConcurrentSkipListMap!
					...
					String result = new String(HTTP.doRequest("GET", "http://"+ masterAddrAndPort + "/workers", null).body());
	...
	// EC 3 checker periodically generate list of current tables, and row keys and hash of each
			...
			try {
                while (true){
					if (workersCount >= 3){
						...
						// first connection: invoke on the next lower worker to get current list of tables (in lowerTableList)
						...
						String result = new String(HTTP.doRequest("GET", "http://"+ ipAndPort + "/replica", null).body());
						String[] pieces = result.split("\n");
						...
						// then, invoke stream reads on each table to get the row keys and hash of each row
						for (String tableName : lowerTableList){
							...
							URL urlReq = new URL("http://" + ipAndPort + "/replica/" + java.net.URLEncoder.encode(tableName, "UTF-8"));
							HttpURLConnection conn = (HttpURLConnection) urlReq.openConnection();
							conn.setDoInput(true);
							conn.setRequestMethod("GET");
							conn.setRequestProperty("Worker", id);
								...
								if (wrapAround){
									if (beginning){
										if (lineSplit[0].compareTo(endKey) < 0){
											if (!currTable.containsKey(lineSplit[0]) || currTable.get(lineSplit[0]).hashCode() != Integer.parseInt(lineSplit[1])){
							...
							// now, we have the rowToGet list that saves all row keys in the table that we need to get
							if (!tables.containsKey(tableName) && rowToGet.size() > 0){
								...
								streams.put(tableName, new BufferedOutputStream(Files.newOutputStream(outputFile, CREATE, APPEND)));
							}
	...
	public static void main(String[] args) throws IOException {
		...
		// PUT /data/<T>/<R>/<C> should set column C in row R of table T to the (possibly binary) data in the body of the request
		put("/data/:table/:row/:col", (req, res) -> {
			...
			// 	URL urlReq = new URL("http://" + worker.nextHigherIpAndPort + "/data/" + tableName + "/" + rowName + "/" + colName);
			// 	HttpURLConnection conn = (HttpURLConnection) urlReq.openConnection();
			...
			// 	conn.setRequestProperty("Connection", "close");
			// 	conn.setRequestProperty("Content-Length", String.valueOf(req.bodyAsBytes().length));
			...
			// 	urlReq = new URL("http://" + worker.nextTwoHigherIpAndPort + "/data/" + tableName + "/" + rowName + "/" + colName);
			// 	conn = (HttpURLConnection) urlReq.openConnection();
			// 	conn.setDoOutput(true);
			// 	conn.setRequestMethod("PUT");
			// 	conn.setRequestProperty("Host", worker.id);
			...
			if (req.queryParams() != null && !req.queryParams().contains("forwarded") && worker.workersCount >= 3){
				HTTP.doRequest("PUT", "http://" + worker.nextHigherIpAndPort + "/data/" + java.net.URLEncoder.encode(tableName, "UTF-8") + "/" + java.net.URLEncoder.encode(rowName, "UTF-8") + "/" + java.net.URLEncoder.encode(colName, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
				HTTP.doRequest("PUT", "http://" + worker.nextTwoHigherIpAndPort + "/data/" + java.net.URLEncoder.encode(tableName, "UTF-8") + "/" + java.net.URLEncoder.encode(rowName, "UTF-8") + "/" + java.net.URLEncoder.encode(colName, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
			}
			...
			if (!worker.tables.containsKey(tableName)){
				...
				worker.streams.put(tableName, new BufferedOutputStream(Files.newOutputStream(outputFile, CREATE, APPEND)));
			}
		...
		// GET /data/<T>/<R>/<C> should return the data in column C of row R in table T if the table, row, and column all exist; if not, it should return a 404 error
		...
		// return HTML pages (content type text/html. The first route should return a HTML table with a row for each data table on the worker; each row should contain a) the name of the table – say, XXX – with a hyperlink to /view/XXX, and b) the number of keys in the table.
		get("/", (req, res) -> {
			// EC 1
			worker.updateRequestReceived();
			...
			for (Map.Entry<String, TreeMap<String, Row>> tableEntry : worker.tables.entrySet()) {
				...
				sb.append("<tr><td><a href=\"" + link + "\">" + tableEntry.getKey() + "</td><td>" + tableEntry.getValue().size() + "</td></tr>");
			}
			sb.append("</table>");
		...
		get("/view/:table", (req, res) -> {
			...
			if (worker.tables.containsKey(tableName)){
				...
				// get the rows starting index, by default it's 0, or if query param has startRow
		...
		// iterate over all the local entries in table XXX, serialize each entry with Row.toByteArray() and then send the entries back, each followed by a LF character (ASCII code 10). After the last entry, there should be another LF character to indicate the end of the stream.
		...
		// the body will contain one or multiple rows, separated by a LF character. The worker should read the rows one by one and insert them into table XXX; existing entries with the same key should be overwritten.
		put("/data/:table", (req, res) -> {
			...
			// EC 2
			// if (!req.headers().contains("forwarded") && worker.workersCount >= 3){
			// 	URL urlReq = new URL("http://" + worker.nextHigherIpAndPort + "/data/" + tableName);
			// 	HttpURLConnection conn = (HttpURLConnection) urlReq.openConnection();
			// 	conn.setDoOutput(true);
			// 	conn.setRequestMethod("PUT");
			// 	conn.setRequestProperty("Host", worker.id);
			...
			// 	urlReq = new URL("http://" + worker.nextTwoHigherIpAndPort + "/data/" + tableName);
			// 	conn = (HttpURLConnection) urlReq.openConnection();
			// 	conn.setDoOutput(true);
			// 	conn.setRequestMethod("PUT");
			// 	conn.setRequestProperty("Host", worker.id);
			...
			if (req.queryParams() != null && !req.queryParams().contains("forwarded") && worker.workersCount >= 3){
				HTTP.doRequest("PUT", "http://" + worker.nextHigherIpAndPort + "/data/" + java.net.URLEncoder.encode(tableName, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
				HTTP.doRequest("PUT", "http://" + worker.nextTwoHigherIpAndPort + "/data/" + java.net.URLEncoder.encode(tableName, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
			}
		...
		// the body will contain another name YYY, and the worker should rename table XXX to YYY (and the corresponding log file from XXX.table to YYY.table. The worker should return a 404 status if table XXX is not found, and a 409 status if table YYY already exists.
		put("/rename/:table", (req, res) -> {
			...
			// EC 2
			// if (!req.headers().contains("forwarded") && worker.workersCount >= 3){
			// 	URL urlReq = new URL("http://" + worker.nextHigherIpAndPort + "/rename/" + tableName);
			// 	HttpURLConnection conn = (HttpURLConnection) urlReq.openConnection();
			// 	conn.setDoOutput(true);
			// 	conn.setRequestMethod("PUT");
			// 	conn.setRequestProperty("Host", worker.id);
			...
			// 	urlReq = new URL("http://" + worker.nextTwoHigherIpAndPort + "/rename/" + tableName);
			// 	conn = (HttpURLConnection) urlReq.openConnection();
			// 	conn.setDoOutput(true);
			// 	conn.setRequestMethod("PUT");
			// 	conn.setRequestProperty("Host", worker.id);
			...
			// 	HTTP.doRequest("PUT", "http://" + worker.nextHigherIpAndPort + "/rename/" + java.net.URLEncoder.encode(oldTable, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
			// 	HTTP.doRequest("PUT", "http://" + worker.nextTwoHigherIpAndPort + "/rename/" + java.net.URLEncoder.encode(oldTable, "UTF-8") + "?forwarded=true", req.bodyAsBytes());
			// }
			...
			// rename the file (replace existing in case a .table exists but has not associated table entry?) all tables should have .table and all .table should have table. Not necessary for REPLACE_EXISTING
		...
		// If a table with this name exists, the body of the response should contain the number of rows in that table (as an ASCII string); otherwise, you should send a 404 error code.
