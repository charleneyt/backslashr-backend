...
public interface Response {
  ...
  // server to send a header line "Cookie: abc=def". This method can be called multiple times
  // with the same header name; the result should be multiple header lines. type(X) should
  // be the same as header("Content-Type", X). If write() has been called, these methods
  ...
  void redirect(String U, int c) throws RedirectException;

  void redirect(String U) throws RedirectException;

  void halt(int status, String reason) throws HaltException;
};
