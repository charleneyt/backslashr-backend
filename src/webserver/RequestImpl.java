...
class RequestImpl implements Request {
  ...
  // When this method is first called for a given request, your server should check whether the request included a cookie with the name SessionID and the value of that cookie is currently associated with a Session object. 
  ...
  // otherwise, it should 
    ...
    // 4) add a Set-Cookie header to the response that sets the SessionID cookie to the chosen session ID, and 
    // 5) return the Session object. 
  // If the method is called again while the server is still handling the same request, it should return the same Session object. 
  // When the method is never called, no session objects or SessionID cookies should be created.
	@Override
