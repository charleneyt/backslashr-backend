...
//	public static int NUM_WORKERS = 30;
	public static int NUM_WORKERS = 256;
	static String CRLF = "\r\n";
...
//                // System.out.println(securePort);
	...
	public class Worker implements Runnable {
			...
			try {
				...
				while (true) {
					...
					if (instanceSecureType) {
						addlStream = true;
						boolean parse = false;
						// for HTTPS socket, use SNIInspector to see if host has cert to establish
						// secure connection
							...
							if (hostName != null && hostTable.containsKey(hostName.getAsciiName())) {
								try {
									...
									if (record.jksFile != null && record.password != null) {
										sock = getSSLContext(record.jksFile, record.password).getSocketFactory()
												.createSocket(sock, inspector.getInputStream(), true);
										addlStream = false;
									} else {
										// covering host('foo.com', null, null) call
										sock = getSSLContext(DEFAULT_JKS, DEFAULT_PWD).getSocketFactory()
												.createSocket(sock, inspector.getInputStream(), true);
										addlStream = false;
									}
								} catch (Exception e) {
									// anytime the validation fails, fall back to default cert and password
									sock = getSSLContext(DEFAULT_JKS, DEFAULT_PWD).getSocketFactory().createSocket(sock,
											inspector.getInputStream(), true);
									addlStream = false;
									e.printStackTrace();
								}
							} else {
		...
		/**
		 * listens and generating response message to client, closes when client has
		 * sent over EOF
		 * 
		 * @throws Exception
		 */
		...
		/**
		 * helper method that keeps reading until meeting EOF when getting double CRLF,
		 * calls processRequest() to parse the info, and sends out response
		 * 
		 * @throws Exception
		 */
		private void executeUntilShutDown() throws Exception {
			...
			// booleans to remember status (only proceed to after line 123 if there are
			// double CRLF)
			...
			// If the header is not present, keep reading until the client closes its half
...
//                fw.flush();
		...
		private void parseRequest() throws IOException {
			...
			// Completeness check
			// 400 Bad Request if the method, URL, protocol header are missing from the
			// request;
...
//            	System.out.println("request string is: " + requestString);
			...
			if (headers.containsKey("content-length")
					&& (contentLength = Integer.valueOf(headers.get("content-length").trim())) > 0) {
			...
			// protocol check
			// 505 HTTP Version Not Supported if the protocol is anything other than
			// HTTP/1.1
			...
			case "HEAD":
				...
				break;
			// 501 Not Implemented if the method is something other than GET, HEAD, POST, or
			// PUT
			default:
				statusCode = 501;
			...
			// check if there's cookie header with with the name SessionID and the value of
			// that cookie is currently associated with a Session object
			...
			if (r != null) {
				...
				// status,
				// or if the passed If-Modified-Since date is invalid (date later than current
				// time),
				...
				if (headers.containsKey("if-modified-since")) {
					try {
						ZonedDateTime lastModifiedSince = ZonedDateTime.parse(headers.get("if-modified-since"),
						...
						if (!lastModifiedSince.isAfter(ZonedDateTime.now())
								&& lastModifiedSince.isAfter(fileLastModified)) {
							// Q: should the header contain Last-Modified: ?
							// A: no need,
							// https://edstem.org/us/courses/24571/discussion/1739948?comment=3973780
							statusCode = 304;
		...
		/**
		 * helper method for dynamic routing processing, save the route if there's a
		 * match
		 * 
		 * @param method
		 * @param path
		 * @return true if there's an existing routing table entry matches with method
		 *         and path
		 */
		private boolean matchRoute(String method, String path) {
			...
			for (RoutingTableEntry entry : routingTable) {
				if (entry.method.equals(method) && (entry.host == null || entry.host.equals(headers.get("host")))) {
					try {
		...
		/**
		 * helper method for dynamic routing sending response instantiate Request and
		 * Response objects, and pass them to the handle method on the Route. If this
		 * throws an exception, return the 500 response as required. If not, set the
		 * Content-Length header and then write out the headers; then write out the
		...
		private void useRouteAndSendResponse() throws IOException {
			...
			request = new RequestImpl(method, url, protocol, headers, qparams, params,
					(InetSocketAddress) sock.getRemoteSocketAddress(), messageBody, serverInstance);
			...
			try {
				...
				// if halt() is called in before stage, immediately stops the request and send
				// message
		...
		/**
		 * helper method that sets type header if non exist, sends response header, and
		 * writes body if method is not HEAD, and there is content
		 * 
		...
		public void sendDynamicResponse(boolean calledFromWrite) throws IOException {
			...
			if (!calledFromWrite) {
				// only send messageBody if content-length is greater than 0, and method is not
				// HEAD
		...
		private void sendStaticResponse() throws Exception {
			...
			if (statusCode != 200) {
				...
				// No message body sent for Error 304? per
				// https://datatracker.ietf.org/doc/html/rfc2616#section-14.25
				// or do it like all other error conditions per instruction
				String message = "HTTP/1.1 " + reasonPhrase + CRLF + "Server: " + serverName + CRLF
						+ "Content-Type: text/plain" + CRLF + "Content-Length: " + reasonPhrase.length() + CRLF + CRLF;
				...
				// Use Stream to open the file, remember to flush pw before sending
				// messageBody!!
				FileInputStream fileStream = new FileInputStream(filePath);
				int contentLength = fileStream.available();

				// need to check range if method is GET, Content-Length will change,
				// also 206 Partial Content should be returned as status, Content-Range needs to
				// be sent in header (bytes 42-1233/1234)
				...
				if (headers.containsKey("range") && "GET".equals(method)) {
					...
					if (ranges == null || ranges.size() == 0) {
						// go with 416, write full file in body with Content-Range "bytes
						// */contentLength"
						String message = protocol + " 416 Range Not Satisfiable" + CRLF + "Server: " + serverName + CRLF
								+ "Content-Type: " + contentType + CRLF + "Content-Length: " + contentLength + CRLF
						...
						String partialHeader = protocol + " 206 Partial Content" + CRLF + "Server: " + serverName + CRLF
								+ "Accept-Ranges: bytes" + CRLF + "Content-Type: " + contentType + CRLF
								+ "Content-Length: " + range[2] + CRLF + "Content-Range: bytes " + range[0] + "-"
						...
						int index = 0;
						messageSize += ranges.size() * (BOUNDARY_DELIMITER.length() + 2 + 43
								+ String.valueOf(contentLength).length() + contentType.length())
								+ BOUNDARY_DELIMITER.length() + 2;
						String partialHeader = protocol + " 206 Partial Content" + CRLF + "Server: " + serverName + CRLF
								+ "Accept-Ranges: bytes" + CRLF + "Content-Length: " + messageSize + CRLF
		 ...
		 * @return list of Integer[] - (start, end, length), return null if range is
		 *         invalid
		 */
		private List<Integer[]> getRangesList(String request, int totalSize) {
			...
			if (request != null && request.matches(regexPattern)) {
				...
				if (!matchOfRange.matches())
					return null;
				// match is guaranteed since checked in if statement
				String[] ranges = matchOfRange.group(1).split(","); // here, matched is a string array
				// then, get parsed group from each of the range, that's also guaranteed since
				// initial matches succeeded
				...
				// fill in the missng index accordingly, and check if it's valid (both >= 0,
				// start <= end, and start > lastEnd)
				for (String range : ranges) {
					...
					if (!matchOfNum.matches()) {
						...
						if (begin >= 0 && begin > lastEnd && begin <= end) {
							...
							messageSize += end - begin + 1 + String.valueOf(begin).length()
		...
		/**
		 * helper method to send partial messages (header and content type was sent
		 * before calling this)
		 * 
		 * @param fileStream
		 * @param range
		 * @param contentLength
		 * @param index
		 * @param multi         boolean value indicating whether we are sending multi
		 *                      range sets response
		 * @return index after sending set bytes of data
		 * @throws IOException
		 */
		private int sendPartialMessage(FileInputStream fileStream, Integer[] range, int contentLength, int index,
