...
public class Master {
	...
	public static String workerTable() {
		...
		while (iter.hasNext()) {
			...
			if (workerEntry.lastPinged == -1 || workerEntry.lastPinged + 15000 < System.currentTimeMillis()) {
				...
				sb.append("<tr><td>" + workerEntry.id + "</td><td>" + workerEntry.ip + "</td><td>" + workerEntry.port
						+ "</td><td><a href=\"http:/" + link.toString() + "\">http:/" + link.toString() + "</td></tr>");
			}
		}
	...
	public static void registerRoutes() {
		// http://xxx/ping?id=yyy&port=zzz input will be coming from Worker, with
		// queryparams id and port
		get("/ping", (req, res) -> {
			// The request should return a 400 error if the ID and/or the port number are
			// missing, and the string OK otherwise.
		...
		get("/workers", (req, res) -> {
			// return k + 1 lines of text, separated by LFs; the first line should contain
			// k, and each of the following lines should contain an entry for a different
			// worker, in the format id,ip:port
			if (lastUpdated == 0 || lastUpdated + 5000 < System.currentTimeMillis()) {
				...
				while (iter.hasNext()) {
					WorkerEntry workerEntry = iter.next();
					// When a worker has not made a /ping request within the last 15 seconds, it
					// should be considered inactive, and be removed from the list
					if (workerEntry.lastPinged == -1 || workerEntry.lastPinged + 15000 < System.currentTimeMillis()) {
						workers.remove(workerEntry.id);
					} else {
